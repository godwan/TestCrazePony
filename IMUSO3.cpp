/**************************************************************************************
Filename:	IMUSO3.c
Description: 姿态融合算法，Part of this algrithom is referred from pixhawk.
Author:		JeromeWan
***************************************************************************************/
#include "IMU.h"
#include "IMUSO3.h"
...
static float q3q3;
static int bFilterInit = 0;
static  int isCalibrating = 1;// 0  no   1 yes

//imu_t imu;
//int imuCaliFlag = 0;
/**************************************************************************************
函数名：void calculatingAttitudeAngle(double* p,int init)
描  述：姿态解算算法主入口
***************************************************************************************/
void calculatingAttitudeAngle(double* p,int init)
{
	...
	if(init)
	{
		...
		dq0 = 0.0f, dq1 = 0.0f, dq2 = 0.0f, dq3 = 0.0f;	/** quaternion of sensor frame relative to auxiliary frame */
		gyro_bias[3] = { 0 }; /** bias estimation */
	...
	if(imu.ready) 
	{
	    for (i = 0; i<3; i++)
		{
			imu.accRaw[i] = (float)p[i];
			imu.gyroRaw[i] = (float)p[3+i];		//deg/s
			imu.magRaw[i] = (float)p[6+i];
		}
...
/**************************************************************************************
函数名：float invSqrt(float number)
描  述：求平方根的倒数
...
/**************************************************************************************
函数名：void NonlinearSO3AHRSinit(float ax, float ay, float az, float mx, float my, float mz)
描  述：Using accelerometer, sense the gravity vector.
...
void IMUSO3Thread(void)
{
    //! Time constant
    float dt = 0.01f;		//s
    //static int tPrev=0,startTime=0;	//us
    //int now = 0;
    int i;
